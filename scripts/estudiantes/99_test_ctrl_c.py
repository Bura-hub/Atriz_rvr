#!/usr/bin/env python3
r"""Prueba 99 - ¿Para el robot cuando pulsas Ctrl-C?
...
🔴 PERO EL WATCHDOG ES SOLO DE LOS MOTORES. Del LIDAR no sabe nada. Son dos
   cosas distintas y conviene tenerlo claro antes del ejercicio 3:
     ...
     final del programa   ¿para el robot?      ¿apaga el barrido del LIDAR?
     -------------------  -------------------  ---------------------------
     Ctrl-C  (SIGINT)     si (cierre limpio)   SI  <- atriz.py lo captura
     Ctrl-\  (SIGQUIT)    si (cierre limpio)   SI  <- capturado
     ...
     fin normal / error   si (cierre limpio)   SI  <- red de `atexit`
     kill -9  (SIGKILL)   si, por el watchdog  NO  <- no se puede capturar
     caida dura           si, por el watchdog  NO  <- ni manejador ni atexit
     sin corriente        si, no hay corriente NO

   Las seis primeras acaban llamando a /stop_scan. Las tres ultimas NO SE
   PUEDEN capturar desde Python —medido: con SIGKILL, SIGSEGV, SIGABRT o un
   `os._exit()` NO corre ni el manejador ni `atexit`—, asi que ahi el barrido
   se queda encendido (11.8 Hz en vez de 2.7, indefinidamente) y solo lo apaga
   el profesor con `atriz-escaneo off`.
...
#      Prueba tambien Ctrl-\ (SIGQUIT) mientras te dice que esperes: tiene que
#      cerrar igual de bien, porque tambien esta capturada.
